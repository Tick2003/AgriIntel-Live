...
\begin{titlepage}
    ...
    \vspace{0.5cm}
    \LARGE
    A Multi-Agent Approach to Live Commodity Tracking, Price Forecasting, and Supply Chain Analytics
...
Agriculture in India is fraught with price volatility and information asymmetry. \textbf{AgriIntel} is an advanced market intelligence system designed to bridge this gap using rigorous Data Science and a Multi-Agent Architecture.

This project implements a multi-agent AI system featuring:
1. \textbf{XGBoost Forecasting} with Auto-Tuning and Technical Indicators.
2. \textbf{KD-Tree Anomaly Detection} for real-time market shock and supply drop detection in metric spaces.
3. \textbf{Computer Vision (CNNs)} for structural quality grading of produce.
4. \textbf{Graph Algorithms (NetworkX)} for calculating net-profit logistics and transport routing.
5. \textbf{NLP Sentiment Analysis} to quantify news impact on prices.
...
\section{Objectives}
The primary objective of AgriIntel is to develop a live dashboard providing:
\begin{itemize}
    \item \textbf{Deep Learning Forecasts}: 30-day ahead price predictions using Auto-Tuned XGBoost.
    \item \textbf{Anomaly Detection}: KD-Tree based classification of market shocks.
    \item \textbf{Logistics \& Quality Analytics}: Net-profit routing via graph algorithms and crop grading via Computer Vision.
...
Instead of static thresholds, we employ \textbf{KD-Trees in Metric Spaces} to classify market days into regimes and detect shocks. By mapping historical price velocity, volatility, and supply drops into a multi-dimensional space, the KD-Tree enables $O(\log n)$ nearest-neighbor lookups to flag anomalous market behavior instantly.

\section{Computer Vision \& Graph Algorithms}
To extend intelligence into the supply chain, the system utilizes:
\begin{itemize}
    \item \textbf{Computer Vision (CNNs)}: For structural quality grading (Grade A/B/C) based on visual data of the produce.
    \item \textbf{Graph Algorithms (NetworkX)}: Modeling markets as nodes and highways as weighted edges to run Dijkstra's algorithm, yielding the most profitable logistics routes by subtracting fuel costs from arbitrage spreads.
\end{itemize}
...
\begin{enumerate}
    \item **Data Ingestion Module**: Must fetch/operationalize daily price and arrival data.
...
The system utilizes a "Multi-Agent Swarm" design where each module acts independently and asynchronously:
\begin{enumerate}
    \item \textbf{DataHealthAgent}: The Gatekeeper. Checks data quality and scraper success rates.
    \item \textbf{ForecastingAgent}: The Analyst. Runs XGBoost ML models.
    \item \textbf{AnomalyDetectionEngine}: The Watchdog. Uses KD-Trees for outliers.
    \item \textbf{ArbitrageAgent \& Logistics}: The Trader. Scans regional corridors and graph networks for profit.
    \item \textbf{MarketRiskEngine}: The Risk Manager. Aggregates signals into a 0-100 score.
    \item \textbf{DecisionAgent}: The Executive. Issues Buy/Sell/Hold signals.
\end{enumerate}

\section{Premium UI Engineering (DOM-Level CSS)}
To achieve a "Bloomberg-Terminal" aesthetic within Streamlit's restrictive sandbox, the platform utilizes advanced DOM-level CSS injection. By targeting internal Emotion-generated \texttt{data-testid} attributes (e.g., \texttt{[data-testid="collapsedControl"]}), the system successfully hides default text fallbacks and implements backdrop-blur glassmorphism.
...
\section{Agent Logic: KD-Tree Anomaly Detection}
The \texttt{AnomalyDetectionEngine} uses \textbf{KD-Trees} trained on historical volatility and shock data to find nearest neighbors in the metric space.
\begin{lstlisting}[language=Python]
def detect_shocks(self, current_data):
    # Vectorize current market state
    point = [current_data['volatility'], current_data['supply_drop']]
    
    # Query KD-Tree for nearest historical neighbors
    dist, ind = self.kdtree.query([point], k=5)
    
    # If distance to known stable states is high -> Shock
    is_shock = np.mean(dist) > self.shock_threshold
    return {"is_shock": is_shock, "severity": np.mean(dist)}
\end{lstlisting}
...
# --- PAGE 6: MODEL PERFORMANCE ---
    ...
    if len(data) > 60:
        ...
        # 3. Generate ML Forecast (Pilot Mode 'Past' Prediction)
